...
DECLARE
  ...
  v_max_code bigint;
BEGIN
  -- Only use order_number (system-generated) to compute next; ignore tracking_code/invoice_number
  -- which may contain user-entered barcodes that are large numbers.
  SELECT GREATEST(
    COALESCE((SELECT MAX((order_number)::bigint) FROM public.orders WHERE order_number ~ '^[0-9]{1,9}$'), 0),
    9999
  ) INTO v_max_code;
...
-- Reset sequence to proper baseline
SELECT setval('public.order_number_seq', GREATEST(
  COALESCE((SELECT MAX((order_number)::bigint) FROM public.orders WHERE order_number ~ '^[0-9]{1,9}$'), 9999),
  9999
), true);